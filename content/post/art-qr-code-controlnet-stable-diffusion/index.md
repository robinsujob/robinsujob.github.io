...
## Stable Diffusion Practical Tips / Stable Diffusion 实战技巧
...
#### Basic Concepts of Prompts / Prompt 的基本概念
...
#### Common Prompt Misconceptions / Prompt 的误区
...
#### Adjusting Prompt Weights / 调整提示词的权重

- Default weight is 1, decreasing left to right / 词缀的权重默认值都是 1，从左到右依次减弱
- Prompt weights significantly affect results / 提示词权重会显著影响画面生成结果
- Use parentheses+colon+number for weights，写法如 `(one girl:1.5)` / 通过小括号+冒号+数字来指定提示词权重
...
#### Mind the Prompt Order / 注意提示词的顺序

- Landscape tags first = smaller characters / 比如景色 Tag 在前，人物就会小，相反的人物会变大或半身
- Correct order and syntax produce better results / 选择正确的顺序、语法来使用提示词
...
#### Emojis in Prompts / Prompt 中的 Emoji

- Emojis work well for expressions，对于特定的人脸表情或动作，可通过添加 emoji 图来达到效果 / Prompt 支持使用 emoji，且表现力较好
- Prioritize emojis over complex syntax / 为了防止语义偏移，优先考虑 emoji
...
#### Recommended Camera Angle Prompts / 视角 Prompt 推荐

| Parameter / 参数 | Description / 说明 |
|------|------|
| extreme closeup / extreme closeup | Face close-up / 面部特写 |
| close up / close up | Head / 头部 |
| medium close up / medium close up | ID photo / 证件照 |
| medium shot / medium shot | Half body / 半身 |
| cowboy shot / cowboy shot | No legs / 不带腿 |
| medium full shot / medium full shot | No feet / 不带脚 |
| full shot / full shot | Full body / 全身 |
...
#### Which Parameters to Adjust / 哪些参数需要调整
...
#### How to Find Optimal Parameters / 如何对比寻找最佳参数
...
#### Image Size Optimization / 图片尺寸优化

- Quality is not directly tied to size.
- Size influences subject category/图片内容，因为它潜在代表选择的类别（比如竖屏人物，横屏风景，小分辨率表情包等）。
...
#### Optimizing Multi and Wide Character Generation / 优化多人物 / 宽幅单人物的生成
...
## Generating Artistic QR Codes with ControlNet / 借助 ControlNet 生成艺术二维码

### Step 1: Optimize the QR Code / Step1：优化二维码
...
### Step 2: Create the Base QR Code / Step2：制作基础二维码
...
- **Anthony's QR Toolkit**：Integrated QR code generation and optimization tool in Webui / 整合在 Webui 的 QRCode 生成与优化工具
...
### Step 3: Determine the Art Style / Step3：确定艺术风格
...
### Step 4: Import QR Code into ControlNet / Step4：在 ControlNet 中导入二维码
...
- **"Enable" button**：Check to ensure ControlNet is active during generation / “启用” 按钮：勾选以确保 ControlNet 在图片生成过程中生效
- **Model selector**：Select "control_v1p_sd15_qrcode_monster" to strengthen QR code control / 模型选框：请选择 "control_v1p_sd15_qrcode_monster" 来加强二维码的控制力度
- **Control weight**：For qrcode_monster model, we recommend setting between 1.1-1.6 / 控制权重：对于 qrcode_monster 模型，我们建议设置在 1.1-1.6 之间
- **Guidance start/end timing**：Start timing recommended between 0-0.1, end timing recommended at 1 / 引导介入/终止时机：介入时机建议在 0-0.1 之间，终止时机建议为 1
...
- **Sampling steps**: recommended 30-50, default 20 is insufficient for high-quality QR code images / 迭代步数：建议在 30-50 之间，默认值 20 不足以引导生成一个高质量的二维码图片
- **Width/Height**: recommended to send the original QR code aspect ratio from ControlNet / 宽度/高度：建议直接从 ControlNet 发送二维码原图的宽高比至上方
...
- ControlNet control weight / ControlNet 控制权重
- ControlNet guidance start/end timing / ControlNet 引导介入/终止时机
...
### Appendix 1: Choosing ControlNet QR Code Models / 附录1：ControlNet QRCode 模型的选择
...
### Appendix 2: How to Use the Stable Diffusion AI Drawing Solution / 附录2：如何使用 Stable Diffusion AI 绘图解决方案
