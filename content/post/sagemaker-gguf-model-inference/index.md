...
## Amazon SageMaker AI Model Deployment Options / Amazon SageMaker AI 模型部署的选项
...
- Have a web server listening on port 8080. / 容器必须有一个在端口 8080 上监听的 Web 服务器。
- Accept POST requests to /invocations and /ping endpoints. Requests must return within 60 seconds with a maximum size of 6 MB. / 容器必须接受对 /invocations 和 /ping 实时端点的 POST 请求。发送到这些端点的请求必须在 60 秒内返回，且最大大小为 6 MB。
...
## llama.cpp and GGUF / llama.cpp 与 GGUF
...
## Hosting GGUF Models in SageMaker AI Using BYOC / 使用 BYOC 方式在 SageMaker AI 中托管 GGUF 模型
...
- From HuggingFace, download a GGUF model and upload it to S3. This blog uses Llama 3 8B GGUF as an example. / 从 HuggingFace 中下载 GGUF 模型，并上传至 S3，本博客将采用 Llama 3 8B 的 GGUF 模型为例。
- Prepare the key BYOC files in the Notebook: Dockerfile, main.py, requirements.txt, serve, server.sh. / 在 Notebook 中准备 BYOC 所需关键文件：Dockerfile、main.py、requirements.txt、serve、server.sh
- Create a custom Docker image and push it to Amazon ECR. / 创建自定义 Docker 镜像，并将 Docker 镜像上传到 Amazon ECR
- Create an Amazon SageMaker Model in the Notebook and deploy it to an inference endpoint. / 在 Notebook 中创建 Amazon SageMaker Model 并部署至 Inference Endpoint
- During endpoint startup, download the GGUF model from S3 to the specified location in the container. / 推理端点运行阶段会从 S3 桶中下载 GGUF 模型至容器指定位置
- Test the endpoint by invoking inference with the SageMaker SDK. / 测试端点，使用 SageMaker SDK 请求调用端点进行推理

### 1. Create Amazon SageMaker AI Notebook / 1. 创建 Amazon SageMaker AI Notebook
...
### 2. Build BYOC Code / 2. 构建 BYOC 代码
...
#### 2.1 Dockerfile / 2.1 Dockerfile 文件
...
- **MODELPATH**: Specifies the model filename used at startup. / **MODELPATH**：用于指定模型启动时的名称
- **BUCKET** and **BUCKET_KEY**: Define the Amazon S3 bucket name and object key for the model file. / **BUCKET** 和 **BUCKET_KEY**：定义存储模型文件的 Amazon S3 存储桶名称及对象名称

在构建 Dockerfile 时，请特别留意以下关键环境变量：

#### 2.2 main.py / 2.2 main.py 文件
...
- Port configuration: main.py defaults to port 8080, connecting to llama.cpp on port 8081 / 端口配置：main.py 默认端口为 8080，连接的 llama.cpp 端口为 8081
- Route changes: /v1/completions changed to /invocations, added /ping health check / 路由修改：/v1/completions 改为 /invocations，新增 /ping 健康检查
- Model loading: Added update_model function for automatic S3 model download / 模型加载：新增 update_model 函数，支持从 S3 自动下载模型文件
...
#### 2.3 SageMaker Inference Entry Point: serve File / 2.3 SageMaker 推理所需入口 serve 文件
...
#### 2.4 llama.cpp Startup Script: server.sh / 2.4 llama.cpp 启动脚本文件 server.sh
...
### 3. Model Deployment and Testing / 3. 模型部署及测试
