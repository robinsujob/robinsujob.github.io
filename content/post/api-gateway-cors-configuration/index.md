...
在启用 CORS 页面，首先配置访问控制允许的方法。为了正确地响应 preflight 请求，OPTIONS 方法已被默认开启。我们勾选需要配置跨域访问的 POST 方法。随后，我们配置访问控制允许的标头和来源，具体允许的值取决于您的后端应用。配置好后点击保存。
...
Enable authorization for the POST method / 为 POST 方法开启授权

![为 POST 方法开启授权](images/cors-config-12.png)

If authorization is enabled for the OPTIONS method, preflight requests will fail / 如果为 OPTIONS 方法开启授权，preflight 请求会出现错误

![如果为 OPTIONS 方法开启授权，preflight 请求会出现错误](images/cors-config-13.png)

Do not enable authorization for the OPTIONS method / 不要为 OPTIONS 方法开启授权
...
在 HTTP API 允许的跨域访问来源端点上测试跨域 POST 请求，可以看到，preflight 的 OPTIONS 请求和实际的 POST 请求都能得到正确响应。这是因为 API Gateway 替我们完成了 HTTP API 对 preflight 请求的响应和 CORS 相关标头的添加。
...
| Type / 类型 | CORS Configuration Method / 跨域配置方法 | Configuration Features / 配置特点 | Authorization Notes / 授权注意事项 |
|---|---|---|---|
| REST API | Configure CORS for specific routes by clicking Enable CORS / 可以为特定路由配置跨域，在该路由下点击启用 CORS | OPTIONS method and response header settings are visible / 可以看到 OPTIONS 方法及其响应标头设置 | Do not enable authorization for OPTIONS method / 不要为 OPTIONS 方法开启授权 |
| HTTP API | Click CORS in the left navigation bar / 点击左侧导航栏的 CORS 进入配置页面 | Preflight handled automatically, not visible in console / preflight 请求由 API Gateway 自动响应，控制台中看不到配置 | Avoid ANY method; if necessary, create an unauthorized OPTIONS method / 避免使用 ANY 方法，如必须使用则另外创建无授权的 OPTIONS 方法 |
