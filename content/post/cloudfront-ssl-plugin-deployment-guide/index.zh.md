...
date: 2023-11-15
description: "China CloudFront SSL 插件部署指南 - 免费 SSL 证书自动化部署与管理。"
...
- **缓存行为** – 通过路径匹配制定缓存行为、启用传输压缩、允许请求方法、访问控制等
- **设置** – 别名（CNAME）/ SSL 证书 / 默认根对象 / 日志记录等
...
- **堆栈名称：** 请为堆栈命名
- **Email：** 输入您的邮箱用于订阅 SNS 邮件提醒，仅支持一个邮箱
- **Domain Name：** 输入域名用于获取 SSL 证书，支持通配符域名和多个域名（英文逗号分隔）
- **SSL Renew Interval Days：** 输入更新证书的周期。Let's Encrypt 证书有效期为 90 天，请确保输入 1-89 内的数字。推荐默认 80 天。
...
- **CloudfrontConsole：** 访问 CloudFront 控制台，快速绑定已颁发的证书
- **ManagementWebURL：** 访问 SwaggerUI，查看或删除 IAM 中已有的 SSL 证书
- **S3BucketURL：** 访问 S3 控制台，下载颁发的 SSL 证书
