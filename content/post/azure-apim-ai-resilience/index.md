...
|------|------|
| *区域* | *功能* |
| **inbound** | Route to pinned backend or round-robin pool |
| ***inbound*** | *读取 `X-AI-Foundry-Target`，有值则路由到指定后端，否则使用 LB Pool 轮询* |
| **outbound** | Return actual backend in response header |
| ***outbound*** | *`X-AI-Foundry-Backend` 响应头返回实际后端名称* |
| **backend** | Direct forward for pinned; auto-retry with exponential backoff for pool |
...
- Video Generation: https://learn.microsoft.com/azure/foundry/openai/concepts/video-generation
